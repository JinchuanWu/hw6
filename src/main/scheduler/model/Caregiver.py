...
class Caregiver:
    ...
    def search_duplicate(self, d):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()

        duplicate_search = "SELECT * FROM Availabilities WHERE Time = %d AND Username = %s"
        try:
            cursor.execute(duplicate_search, (d, self.username))
    ...
    def search_appointments(self):
        ...
        search_all_app = "SELECT appointment_id, Name, Time, Patientsname FROM Reservation WHERE Username = %s ORDER BY appointment_id"
        try:
            cursor.execute(search_all_app, self.get_username())
            result = cursor.fetchall()
            return result
        except pymssql.Error as e:
            raise e
